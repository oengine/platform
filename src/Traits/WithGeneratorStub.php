    ...
    public function getNamespaceReplacement()
    {
        if (!$this->_namespace) {
            ...
            if ($dataInfo) {
                ...
                $this->_namespace = config('platform.namespace.root', config('platform.appdir.root', 'platform')) . '\\' . config('platform.namespace.' . $this->getBaseTypeName(), config('platform.appdir.' . $this->getBaseTypeName())) . "\\" . $this->getStudlyNameReplacement();
            }
        }
